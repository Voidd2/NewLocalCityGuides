...
### Merging
- **Standing user authorization, 2026-09-13:** Codex merges its own completed PRs into `main` by default after relevant checks and independent review. This records the user's explicit instruction and removes the need to ask again for each Codex PR. It does not authorize merging Claude's PRs or bypassing repository protections.

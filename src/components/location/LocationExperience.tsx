...
export function LocationExperience({ location }: { location: LocationData }) {
  ...
  return (
    <div>
          ...
          <div className="relative h-64 md:h-80 bg-gray-200">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-4">
              ...
              <div className="flex items-center gap-3 text-xs text-white/70 mt-1">
                ...
                <span>30-60 {t("minutes")}</span>
              </div>
            </div>
          </div>

          <div className="max-w-7xl mx-auto px-4 py-6">
            <p className="text-sm text-gray-700 mb-6">{location.shortDescription}</p>

            <button
              onClick={() => setState("arrived")}
              className="w-full bg-orange-500 hover:bg-orange-600 text-white font-semibold py-4 rounded-full transition-colors text-base"
            >
              {t("imHere")}
      ...
      {state === "arrived" && (
        <div className="max-w-7xl mx-auto px-4 py-6">
          <h2 className="text-xl font-bold text-navy-800 mb-2">{location.name}</h2>
      ...
      {state === "video" && (
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="relative bg-navy-900 rounded-2xl overflow-hidden aspect-video mb-4">
            {/* Video player placeholder - Cloudflare Stream will be integrated */}
          ...
          <p className="text-xs text-gray-400 text-center mb-4">Video speelt niet automatisch af. Druk op play om te starten.</p>

          <div className="flex gap-2">
            <button
              onClick={() => setState("story")}
              className="flex-1 bg-navy-800 text-white font-semibold py-3 rounded-full text-sm"
            >
              Lees verder
            </button>
            <button
              onClick={() => setState("arrived")}
              className="flex-1 border border-gray-300 text-gray-700 font-semibold py-3 rounded-full text-sm"
            >
              Terug
            </button>
          </div>
      ...
      {state === "story" && (
        <div className="max-w-7xl mx-auto px-4 py-6">
          ...
          <p className="text-sm text-gray-500 mb-4">{location.mainTheme}</p>
          ...
          <div className="flex gap-2">
            <button
              onClick={() => setState("practical")}
              className="flex-1 bg-orange-500 text-white font-semibold py-3 rounded-full text-sm"
            >
              {t("practicalInfo")}
            </button>
            <button
      ...
      {state === "practical" && (
        <div className="max-w-7xl mx-auto px-4 py-6">
          ...
          <button className="w-full bg-navy-800 text-white font-semibold py-3 rounded-full text-sm mb-3">
            {t("showOnMap")}
          </button>

          <button
            onClick={() => setState("arrived")}
            className="w-full border border-gray-300 text-gray-700 font-semibold py-3 rounded-full text-sm"
          >
            Terug naar keuzes
          </button>
        </div>
